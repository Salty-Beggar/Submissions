...
int main() {
    ...
    while (!choicePQueue.empty()) {
        ...
        choicePQueue.pop();
        if (shortestRoutes[curChoice.first] > curChoice.second) {
            shortestRoutes[curChoice.first] = curChoice.second;
            vector<pair<long int, long int>> curAdjList = adjList[curChoice.first];
            for (long int i = 0; i < curAdjList.size(); i++) {
                choicePQueue.push({curAdjList[i].first, curChoice.second+curAdjList[i].second});
            }
        }
    }
